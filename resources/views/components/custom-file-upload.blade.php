...
<div class="relative flex justify-end items-center">
    <div class="flex items-center">
        ...
        <span id="{{ $name }}-file-name" class="mr-4 text-sm text-gray-500"></span>

        <!-- Hidden file input -->
        <input id="{{ $name }}" type="file" name="{{ $name }}" class="hidden" onchange="document.getElementById('{{ $name }}-file-name').textContent = this.files[0] ? this.files[0].name : ''">
    </div>
